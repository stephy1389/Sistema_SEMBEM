...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $("#selectbuscador1, #selectbuscador2, #selectbuscador3, #p_sexo, #p_edo_civil, #p_nacionalidad,"+
          ...
          "#u_permisologia_morb, #u_status, #p_tipo, #pni_color_ojos, #pni_color_piel, #pni_color_cabello, #pni_contextura,"+
          "#selectbuscador4, #pie_status_realizar, #pie_status_entrega, #u_id_pregunta, #u_id_pregunta_nueva")
          .select2();
    });
    ...
    $("[name='resetpassword']").bootstrapSwitch();

    $(document).ready(function(){
        if(document.getElementById("pie_status_realizar").value == 'f'){
            document.getElementById("pie_status_entrega").disabled = true;
            document.getElementById("pie_fecha_retiro").disabled = true;
        }
        else {
            document.getElementById("pie_status_entrega").disabled = false;

            if (document.getElementById("pie_status_entrega").value == 't'){
                document.getElementById("pie_fecha_retiro").disabled = false;
            }
            else{
                document.getElementById("pie_fecha_retiro").disabled = true;
            }
    ...
    function activaInput(){
        $valor = document.getElementById("pie_status_realizar").value;

        if($valor == 't'){
            document.getElementById("pie_status_entrega").disabled = false;
        }
        else{
            $("#pie_status_entrega").val('f');
            $("#select2-pie_status_entrega-container").text("NO ENTREGADO");
            document.getElementById("pie_status_entrega").disabled = true;

            document.getElementById("pie_fecha_retiro").value = null;
            document.getElementById("pie_fecha_retiro").disabled = true;
        }
    }

    function activaInputFechaHora(){
        $valor = document.getElementById("pie_status_entrega").value;

        if($valor == 't'){
            <!--document.getElementById("pie_status_realizar").disabled = true;-->
            document.getElementById("pie_fecha_retiro").disabled = false;
        }
        else{
            document.getElementById("pie_fecha_retiro").value = null;
